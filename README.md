...
Spins up a local L1 (Geth + Teku) and deploys a fresh OP Stack L2 on top of it. No configuration needed.
...
Runs just the local Ethereum L1 chain (Geth + Teku's in-process validator). Useful for L1 development or testing without the OP Stack.
...
### Glamsterdam transition under transfer load

The local fork schedule matches Plataberget but compresses Gloas/Amsterdam from epoch 1536 to epoch 1. The test profile submits roughly 40 native transfers per second and checks that the sequencer and verifier derive matching safe heads from post-Amsterdam L1 blocks.

```bash
# Pull first so image downloads do not consume the pre-fork window.
docker compose --profile l1 --profile l2 --profile test pull
docker compose --profile l1 --profile l2 --profile test down -v
docker compose --profile l1 --profile l2 --profile test up -d

# Follow the transition and return the checker's exit status.
docker compose --profile l1 --profile l2 --profile test logs -f glamsterdam-check &
docker compose --profile l1 --profile l2 --profile test wait glamsterdam-check
```

A successful run ends with `GLAMSTERDAM TRANSITION PASSED`. The transition takes about seven minutes from genesis generation.
...
| `L1_BEACON_URL` | `http://teku:5052` | L1 consensus layer beacon API endpoint |
...
| `L2_CHAIN_ID` | `420120214` | L2 chain ID (matches `glamsterdam-def-0`) |
...
| `OP_DEPLOYER_IMAGE` | `us-docker.pkg.dev/oplabs-tools-artifacts/images/op-deployer:v0.7.1` | Overrides the op-deployer image used by every deployment stage, useful for testing local or PR builds. |
...
| `teku` | L1 consensus layer beacon node with an in-process validator client |
...
| Service | Description |
|---------|-------------|
| `l1-el-ready` | Waits for pre-Amsterdam L1 block production and RPC indexing |
...
| `op-reth` | L2 sequencer execution engine (port 9545) |
| `l2-el-ready` | Waits for op-reth RPC to be responsive |
| `op-node` | L2 sequencer derivation / consensus (port 8547) |
| `op-reth-2` | L2 verifier execution engine (port 9645) |
| `op-node-2` | L2 verifier derivation / consensus (port 8647) |
...
| `transfer-spammer` | Test profile: submits continuous L2 native transfers |
| `glamsterdam-check` | Test profile: verifies matching unsafe and safe heads across Amsterdam |
...
| 8548 | op-batcher | RPC |
| 5058 | blob-api | HTTP |
| 9645 | op-reth-2 (L2 verifier EL) | HTTP RPC |
| 8647 | op-node-2 (L2 verifier) | HTTP RPC |
